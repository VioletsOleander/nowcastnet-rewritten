...
if __name__ == "__main__":
    ...
    configs = {key: value + args.case_type for key, value in configs.items()}
    configs = construct_configs(configs)

    os.makedirs(configs.save_dir, exist_ok=True)

    print("Comparing ONNX and Torch evaluation results")
    onnx_numpy_files = sorted(
        [
            filename
            for filename in os.listdir(configs.onnx_eval_results_path)
            if filename.endswith(".npy")
        ]
    )
    torch_numpy_files = sorted(
        [
            filename
            for filename in os.listdir(configs.torch_eval_results_path)
            if filename.endswith(".npy")
        ]
    )
    ...
    for onnx_numpy_file, torch_numpy_file in zip(onnx_numpy_files, torch_numpy_files):
        ...
        onnx_numpy = np.load(
            os.path.join(configs.onnx_eval_results_path, onnx_numpy_file)
        )
        torch_numpy = np.load(
            os.path.join(configs.torch_eval_results_path, torch_numpy_file)
        )
