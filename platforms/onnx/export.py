...
def prepare_kwargs(configs, args):
    ...
    if not args.dynamic:
        kwargs['f'] = f'{args.artifacts_dir}/{args.output_name}_{configs.image_height}.onnx'
    else:
        kwargs['f'] = f'{args.artifacts_dir}/{args.output_name}_dynamic.onnx'

        batch_dim = Dim('batch_size')
        kwargs['dynamic_shapes'] = {
            'observed_frames': [batch_dim, Dim.STATIC, Dim('image_height'), Dim('image_width')],
