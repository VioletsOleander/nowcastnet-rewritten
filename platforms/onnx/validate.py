...
def get_torch_output(config, numpy_input, noise):
    ...
    torch_model.load_state_dict(torch.load(config.weights_path, weights_only=True))
    torch_model.eval()
...
def get_onnx_output(config, numpy_input, noise):
    ...
    onnx_output = ort_session.run(
        [output_name], {input_name: numpy_input, noise_name: noise}
    )[0]
...
if __name__ == "__main__":
    parser = refine_parser(
        setup_parser("Validate the precision of the ONNX model output")
    )
    ...
    noise = np.random.randn(
        ...
        torch_configs.image_width // 32,
    ).astype(np.float32)
    print(f"noise shape: {noise.shape}")

    torch_output = get_torch_output(torch_configs, numpy_input, noise).cpu().numpy()
    print(f"torch output shape: {torch_output.shape}")

    onnx_output = get_onnx_output(onnx_configs, numpy_input, noise)
    ...
    try:
        ...
        greatest_abs_diff_index = tuple([int(num) for num in indices[0].split(", ")])
        greatest_rel_diff_index = tuple([int(num) for num in indices[1].split(", ")])

        message = "elements in greatest absolute difference:\n"
        message += (
            f"torch_output[{indices[0]}]: {torch_output[greatest_abs_diff_index]:.8f}, "
        )
        message += (
            f"onnx_output[{indices[0]}]: {onnx_output[greatest_abs_diff_index]:.8f}"
        )
        print(message)

        message = "elements in greatest relative difference:\n"
        message += (
            f"torch_output[{indices[1]}]: {torch_output[greatest_rel_diff_index]:.8f}, "
        )
        message += (
            f"onnx_output[{indices[1]}]: {onnx_output[greatest_rel_diff_index]:.8f}"
        )
        print(message)
